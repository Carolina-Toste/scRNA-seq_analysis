...
read_cogent <-
  function(path_to_output = "../../../CogentAP 2.0 analysis results of Syk Chip for Mateus Nov 22",
...
- NOTE: **TODO** - ask Elena/Carolina to double-check that the files I've read in are the same as what they've been working with

-   Read in the other file-types, you will need to put these files in the same folder as this script.
    -   If there are files from multiple chips ensure correct `fn` selected
    -   Adds percentage read values in stats data
    -   Identify the controls and samples
    -   Displays, then removes all Short, Unselected or Undetermined reads from subsequent analysis
    -   To look at a snapshot of the tables use `head(stats)` or `head(gene_info)`
    -   Replace the chip name as needed
